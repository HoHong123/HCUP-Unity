...
> 구성 어셈블리 3개 (빈 껍데기 2개는 2026-08-07 기준 이미 삭제됨)
...
| `HCUP.HUtil.Editor` | Editor | 2 | `Unity.Addressables.Editor`, `HCUP.HUtil`, `HCUP.HDiagnosis` |
| `HCUP.HUtil.Odin.Editor` | Editor (`ODIN_INSPECTOR`) | 1 | `HCUP.HDiagnosis` |

`HCUP.Util.Odin`(Runtime, `ODIN_INSPECTOR`) / `HCUP.Util.Tween`(Runtime, `USE_DOTWEEN`) —
소스 0파일 빈 어셈블리였던 2건은 2026-08-07 기준 저장소에서 이미 삭제 확인됨.
...
1. ~~asmdef 참조 2건이 코드 근거 0건이다~~ — `HCUP.HUtil` 의 `HCUP.HData`·`HCUP.HInspector`,
   `HCUP.HUtil.Editor` 의 `Unity.TextMeshPro(.Editor)` 2종, `HCUP.HUtil.Odin.Editor` 의
   `HCUP.HUtil` 을 grep 재확인 후 전부 제거 (2026-08-07 반영).
