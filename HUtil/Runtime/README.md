...
> 의존: `HCUP.HDiagnosis`
> 동반 어셈블리: `HCUP.HUtil.Editor`, `HCUP.HUtil.Odin.Editor`(`ODIN_INSPECTOR` 조건부)

---

## 요약

이 문서는 **`HCUP.HUtil` 런타임 어셈블리의 현재 내용물**만 다룬다. 예전에 HUtil 에 있던 `AssetHandler` / `Data` / `Scene` / `Time` / `Logger` 계층은 다른 어셈블리(`HCUP.HResource`, `HCUP.HCore`, `HCUP.HDiagnosis`)로 분리되었고 여기에 없다. 옮겨 간 위치는 [HUtil/README.md](../README.md) 의 이관표에 있다.
...
| **Font** | 1 | `UnityEngine.UI.Text` 용 다방향 아웃라인 (8방향) |

**세 시스템 사이에 참조가 없다.** 어느 하나를 지워도 나머지가 컴파일된다. 21 파일 1029 행 (`Samples~` 제외. Animation 254+143, Pooling 510, Font 122)이고, Animation 15 파일 중 14 파일이 20~26 행짜리 정형 파일이다. 별도 문서로 분리할 만한 부피가 아니라 이 README 하나로 충분하다.
...
| `HUtil/Pooling/BasePool.cs` | 230 | 풀 본체. `Stack` + `HashSet` 2컬렉션 |
| `HUtil/Pooling/ClassPool.cs` | 53 | `new()` 제약 C# 클래스 풀 |
| `HUtil/Pooling/ComponentPool.cs` | 84 | `Component` 풀. prefab 유무 분기 |
| `HUtil/Pooling/GameObjectPool.cs` | 71 | `GameObject` prefab 풀 |
| `HUtil/Pooling/ParticlePoolingSystem.cs` | 72 | 파티클 종료 감지 보조 컴포넌트 |
| `HUtil/Font/BetterOutline.cs` | 122 | `Shadow` 파생 아웃라인 (외부 출처 코드) |
...
```csharp
// HUtil/Animation/BaseAnimatorStateRouter.cs:29-44 (주석의 em dash 는 하이픈으로 옮겨 적음)
...
flowchart LR
    ...
    CHK -->|예| ALV{"IsAlive(obj)"}
    ALV -->|아니오| DROP["activatedPool.Remove + Warning<br/>스택에 넣지 않음"]
    ALV -->|예| R2["onReturn 호출 → Push + Remove"]
    R2 --> ST
    X["Discard(obj)"] --> XR["activatedPool.Remove 만"]
    D["Dispose()"] --> D1["pool 전량 onDispose"]
    D --> D2["activatedPool 전량<br/>경고 + onDispose"]
```

**`Stack` 을 쓰는 이유는 캐시 지역성이다.** 최근 반환된 객체가 메모리 캐시에 가깝다 (`BasePool.cs` Dev Log). **`HashSet` 을 쓰는 이유는 중복 반환 검사**다 (`BasePool.cs:111-115`).
...
| `protected virtual IsAlive(obj)` | 재사용 가능 판정. 기본은 `obj != null`, Unity 객체 풀은 fake-null 판정으로 재정의 | `:107` |
| `Return(obj)` | 미보유 객체는 경고 후 무시. 파괴된 객체는 장부에서만 지우고 스택에 넣지 않는다 | `:111-131` |
| `Discard(obj)` | 대여 중 객체를 장부에서만 제거. 대여 중이 아니면 경고 후 `false` | `:136-142` |
| `Dispose()` | 미반납분 경고 + 양쪽 `onDispose` | `:146-156` |
| `protected abstract T Create()` | 파생 클래스의 유일한 필수 구현 | `:160` |
...
| `ComponentPool<T>` | `Component` | prefab 있으면 `Instantiate`, 없으면 `new GameObject(typeof(T).Name, typeof(T))` | `ComponentPool.cs:46-58` |
| `GameObjectPool` | (비제네릭) | `Instantiate(prefab, parent)` | `GameObjectPool.cs:42-46` |

세 클래스 모두 **생성자에서 `Init(initialSize)` 를 호출한다.** 즉 `new ComponentPool<T>(prefab, 20)` 시점에 20개가 즉시 `Instantiate` 된다 (`ComponentPool.cs:36`, `GameObjectPool.cs:32`, `ClassPool.cs:23`).

`ComponentPool` 과 `GameObjectPool` 은 `IsAlive` 를 Unity fake-null 판정(`=> obj`)으로 재정의한다 (`ComponentPool.cs:42`, `GameObjectPool.cs:38`). `ClassPool` 은 기본 판정을 쓴다.

`ParticlePoolingSystem` 은 풀이 아니라 **보조 컴포넌트**다. `Awake` 에서 `ParticleSystem.main.stopAction` 을 `Callback` 으로 강제해(`ParticlePoolingSystem.cs:36-37`) 재생 종료 시 `OnParticleSystemStopped` 를 받고, 이를 `OnStopped` 이벤트로 외부에 넘긴다. `ParticleSystem` 이 없으면 `HLogger.Error` 후 조기 반환한다 (`:31-35`). 풀에 되돌리는 것은 구독자의 책임이다.
...
대각선 오프셋에 `0.707`(≈ 1/√2)을 곱해 대각 방향 거리를 축 방향과 맞춘다 (`BetterOutline.cs:56`). 대각 4개(`:51-59`) + 축 4개(`:61-76`) = 8개 Shadow 다.

`Shader.Find("Text Effects/Fancy Text")` 는 `static readonly` 필드로 1회만 조회하고 (`:24`), `Text` 컴포넌트도 `cachedText` 필드에 캐시한다 (`:79`).
...
2. **핸들러 부재는 `LogError` 1회 후 영구 무시다.** `missingHandlerWarned` 는 리셋되지 않으므로 (`:27`, `:37-38`), 런타임 중 핸들러를 나중에 붙여도 이미 경고가 나간 라우터는 **다시 시도는 하지만 경고는 다시 뜨지 않는다.** 핸들러가 없는 동안에는 `handler != null` 체크를 통과하지 못해 매 이벤트 `GetComponent` 를 다시 부른다. 매 프레임 호출되는 `AnimatorStateUpdateRouter` 에서는 이것이 지속적인 비용이다.
...
5. **`Return` 은 그 풀에서 나온 객체만 받는다** (`BasePool.cs:112-115`). 다른 풀의 객체나 직접 생성한 객체를 넣으면 경고 후 무시된다. 조용히 실패하지는 않지만 반납도 안 된다.
6. **대여 중에 파괴된 객체를 `Return` 하면 스택에 돌아가지 않는다** (`BasePool.cs:119-126`). 장부에서만 지우고 경고를 남기며 `onReturn` 도 부르지 않는다. 다음 `Get` 에서 새 인스턴스가 생성된다. 반환 경로 없이 장부만 정리하려면 `Discard` 를 쓴다.
7. **`Dispose` 는 풀을 비울 뿐 Unity 오브젝트를 파괴하지 않는다** (`:146-156`). `Destroy` 는 `onDispose` 콜백으로 호출측이 넘겨야 한다.
8. **`BetterOutline` 은 `UnityEngine.UI.Text` 전용이다** (`:21`).

### 정리 대상

9. **`asmdef` 파일명과 어셈블리명이 어긋난다.** `HUtil/Editor/Odin` 은 파일명이 `HCUP.Util.Odin.Editor.asmdef` 인데 안의 `name` 은 `HCUP.HUtil.Odin.Editor` 다.
10. **`BetterOutline.OnValidate` 는 `base.OnValidate()` 만 부른다** (`:32-34`). 순수한 빈 오버라이드로 제거 가능하다.
11. **`CountAvaliable` 오타** (`BasePool.cs:42`). `CountAvailable` 이 맞다. 공개 프로퍼티라 이름을 바꾸면 이 풀을 쓰는 코드가 함께 바뀌어야 한다.
12. **`Activates` 가 내부 `HashSet<T>` 을 그대로 노출한다** (`:44`). 외부에서 `activatedPool.Clear()` 를 부르면 풀 회계가 무너진다. `IReadOnlyCollection<T>` 로 좁히는 것이 맞다.
13. **`onCreate` 호출 위치가 `BasePool` 이 아니라 파생 클래스다** (`ClassPool.cs:29`, `ComponentPool.cs:56`, `GameObjectPool.cs:44`). 세 곳에 같은 코드가 반복되고, 새 파생 클래스가 이를 빠뜨리면 조용히 콜백이 죽는다. `BasePool.Create(int)` 쪽으로 올릴 수 있다.
14. **`Samples~/SceneUtil/SceneTester.cs` 는 컴파일되지 않는다.** `Samples~` 는 Unity 가
...
| 새 풀 타입 | `BasePool<T>` 상속 + `protected override T Create()` 구현 (`BasePool.cs:160`). Unity 객체면 `IsAlive` 도 재정의 (`:107`) |
| 풀 상한(최대 개수) 도입 | `BasePool.Get()` (`:93-102`)의 자동 생성 분기 |
| 미반납 추적 강화 | `BasePool.Dispose()` (`:146-156`)의 `activatedPool` 순회. 이미 경고를 찍는다 |
...
### 2026-09-04 :: `BasePool` 에 파괴 객체 처리(`IsAlive`)와 `Discard` 추가

- 이전: `Return` 은 파괴된 객체도 스택에 되돌렸고, 장부만 정리하는 API 가 없었다. 파일 지도의 `BasePool.cs` 는 160 행, `ComponentPool.cs` 79 행, `GameObjectPool.cs` 66 행이었다.
- 현재: `IsAlive` 가상 훅과 `Discard`(2026-09-02 추가)가 생겼고 `ComponentPool` / `GameObjectPool` 이 fake-null 판정으로 재정의한다. 멤버 표의 행 번호가 그에 맞춰 바뀌었다 (`Return` `:106-115` → `:111-131`, `Dispose` `:119-129` → `:146-156`, `Create()` `:133` → `:160`).

### 2026-08-07 :: `BetterOutline` / `ParticlePoolingSystem` 결함 수정

- 이전: `BetterOutline` 헤더가 "8방향 + 4방향 총 12개 Shadow" 라고 적었고, `ModifyMesh` 마다 `Shader.Find` + `GetComponent` 를 호출했다. `ParticlePoolingSystem.Awake` 에는 null 가드가 없어 `ParticleSystem` 이 없으면 `particle.main` 에서 NRE 가 났다.
- 현재: 헤더는 8개로 정정됐고, 셰이더는 `static readonly` 필드, `Text` 는 `cachedText` 필드로 캐시한다. `ParticlePoolingSystem` 은 `HLogger.Error` 후 조기 반환한다.

### 2026-08-07 :: 빈 어셈블리 삭제와 미사용 asmdef 참조 제거

- 이전: `HUtil/Runtime/Odin`(`HCUP.Util.Odin`, `Core/` `Pooling/` `Sound/` 하위 폴더까지 비어 있음)과 `HUtil/Runtime/Tween`(`HCUP.Util.Tween`)이 `.cs` 0개인 빈 어셈블리로 있었다. `HCUP.HUtil` 은 코드에서 쓰지 않는 `HCUP.HData` / `HCUP.HInspector` 를 참조했다.
- 현재: 두 폴더는 삭제됐고, `HCUP.HUtil` 의 참조는 `HCUP.HDiagnosis` 하나다.
