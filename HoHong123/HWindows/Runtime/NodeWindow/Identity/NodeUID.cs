...
namespace HWindows.NodeWindow.Identity {
    ...
    public struct NodeUID : IEquatable<NodeUID> {
        ...
        // deserialize 시 reflection 으로 쓰지 못해 _value=0 으로 복원되는 문제 회피.
        ...
        public bool Equals(NodeUID other) => _value == other._value;
        public override bool Equals(object obj) => obj is NodeUID other && Equals(other);
        public override int GetHashCode() => _value;
        public override string ToString() => $"NodeUID({_value})";
        #endregion

        #region Public - Operators
        public static implicit operator int(NodeUID id) => id._value;
...
//
//   [값 규칙]
//   - _value > 0: 유효한 NodeUID
//   - _value <= 0: NodeUID.None (0 예약, IsValid == false)
//
//   [발급 책임]
...
//     catalog.nodes 에 들어간 NodeUID 3개가 전부 _value=0 으로 복원됨.
...
//     + 결과: serialize 시 YAML 에는 값이 저장되지만, deserialize 시 _value=default(0) 으로 복원.
//   - 수정:
//     + public readonly struct NodeUID -> public struct NodeUID (readonly 제거)
//     + readonly int _value -> int _value (readonly 제거)
