...
1. **`HLogger`** - 로그 진입점. `Debug.Log` 를 직접 부르지 않고 **`LogEntry` 구조체를 만들어
   `OnLogPublished` 이벤트로 발행**한 뒤, 에디터·개발 빌드에서만 콘솔에도 찍는다. 인게임 콘솔(`HUI.DebugConsole.HLogConsole`)은 이 이벤트와 `Application.logMessageReceived` 를 함께 구독한다.
2. **`HDebug`** - `[Conditional("UNITY_EDITOR")]` 스택 트레이스 유틸. 릴리즈 빌드에서는
   **호출 코드 자체가 컴파일러에 의해 제거된다.**

단일 README 로 충분한 규모다(4파일 416행). 두 갈래는 `HDebug → HLogger` 한 방향으로만 결합돼 있다.
...
| 경로 | 행수 | 역할 |
|---|---|---|
| `Logger/HLogger.cs` | 212 | 로그 진입점 + `LogEntry` 구조체 + `OnLogPublished` 이벤트 |
...
public readonly struct LogEntry {
    ...
    public readonly int? TargetInstanceId;     // target ? target.GetInstanceID() : null - 콘솔에서 오브젝트 핑용
}
```

`TargetInstanceId` 는 `GameObject` 참조를 들고 있지 않고 **int id 만** 보관한다(`:98, :111, :124`). 소비측(`HUI/Runtime/HUI/DebugConsole/HLogConsole.Actions.cs:221-230`)이 에디터에서 `EditorUtility.EntityIdToObject`(6000.3 이상) 또는 `InstanceIDToObject` 로 되살린다 - 로그가 오브젝트 수명을 붙잡지 않게 하는 설계다.
...
sequenceDiagram
    ...
    L->>L: new LogEntry(level, DateTimeOffset.UtcNow, message, debug, target 의 InstanceID)
...
구독자는 릴리즈에서도 로그를 받는다(`:164-170`). 사라지는 것은 `UnityEngine.Debug` 출력뿐이다
(`:172-193`). 인게임 콘솔이 릴리즈 빌드에서도 동작하는 근거가 여기다.

### API 표

| API | 레벨 | 비고 |
|---|---|---|
| `Log(message, target, popupActivate)` | `Log` | `popupActivate` 는 **TODO 스텁** (`:105-107`) |
| `Warning(message, target, popupActivate)` | `Warn` | 동일 스텁 (`:118-120`) |
| `Error(message, target, showPopup, debug)` | `Error` | `debug` 는 이 API 에서만 채워진다 |
| `Exception(ex, extra)` | `Error` | `extra` 가 있으면 `"{extra}\n{ex}"` (`:137`) |
| `Throw(ex, extra, doThrow = true)` | `Error` | 로그 후 **실제로 `throw`** (`:146-150`) |
| `Assert(condition, message, target)` | - | `[Conditional]` 2종(`UNITY_EDITOR`, `DEVELOPMENT_BUILD`). 실패 시 `Debug.Assert` (`:152-156`) |
| `SendLogsToServer()` | - | **빈 스텁** (`:158-160`) |
...
1. **`HLogger.Throw` 는 던진다.** `doThrow: false` 를 명시하지 않는 한 호출 지점에서 예외가 나간다
   (`:146-150`). `Awake` 안에서 부르면 그 프레임의 초기화가 중단된다.
2. **`HLogger.Assert` 는 릴리즈에서 호출째 사라진다**(`:152`, `[Conditional]` 2종). 릴리즈에서도
   지켜져야 하는 검증에는 `HLogger.Error` 나 `HLogger.Throw` 를 쓴다 - 패키지 내 다른 모듈들이
   `Assert` 에서 `HLogger` 로 옮겨 온 이유다.
3. **릴리즈에서도 `OnLogPublished` 는 발행된다**(`:164-170`). 구독자를 해제하지 않으면 정적 이벤트가
...
5. **`LogEntry.Timestamp` 는 UTC 다.** `_UtcNow` 가 `DateTimeOffset.UtcNow` 를 반환한다(`:93`). 콘솔 포맷에 오프셋(`zzz`)이 들어가 `+00:00` 으로 표시되며, 로컬 시각이 아니다.
6. **정적 상태는 `SubsystemRegistration` 시점에 리셋된다**(`_ResetStatics`, `:83-89`). Domain Reload 를 끈 에디터에서도 `OnLogPublished` 구독자가 플레이 세션을 넘어 남지 않는다. `logQue.Clear()` 는 큐가 있는 비에디터 빌드에서만 컴파일된다.

### 정리 대상

7. **`logQue` 는 쓰기 전용이다.** `:78` 선언, `:166-167` Enqueue/Dequeue, `:87` Clear 뿐이고 읽는 코드가 없다. 이 큐를 읽을 예정이던 `SendLogsToServer()` 는 빈 스텁이다(`:158-160`). 비에디터 빌드에서 최대 1000건의 `LogEntry` 를 메모리에 붙잡아 두기만 한다.
8. **`LogLevel` 6종 중 3종은 `HLogger` 가 생성하지 않는다.** `HLogger` 가 만드는 값은 `Log`/`Warn`/`Error` 뿐이다(`:98, :111, :124, :138`). `_Publish` 가 private 이라 외부에서 다른 레벨의 엔트리를 발행할 경로도 없다. 소비측 `HUI/.../HLogCellView.cs:62-63`, `HLogConsole.Actions.cs:177-185` 는 이 셋을 분기 처리하고 있어 `HLogger` 경로에서는 **도달 불가 분기**다.
9. **`popupActivate` / `showPopup` 는 세 곳 모두 TODO 스텁이다**(`:105-107, :118-120, :131-133`).
    인자를 주더라도 아무 일도 일어나지 않는다. 인자명도 `Log`/`Warning` 은 `popupActivate`,
    `Error` 는 `showPopup` 으로 갈린다.
10. **네임스페이스와 타입 이름이 같다.** `namespace HDiagnosis.HDebug` 안의 `class HDebug`
...
11. **비에디터 빌드의 private 폴백 2종은 죽은 코드다**(`:92-93`). 호출자가 전부 `[Conditional]` 로
...
| 팝업 연동 | `HLogger` 의 TODO 3곳 (`:106, :119, :132`) - `popupActivate` 경로 구현 |
...
- 이전: `HLogger` 가 `public class` 라 멤버가 전부 static 인데도 암묵적 public 생성자로 `new HLogger()` 가 가능했다. `_UtcNow` 는 이름과 달리 `DateTimeOffset.Now`(로컬 시각)를 반환했다.
- 현재: `public static class HLogger`(`:18`)이고 `_UtcNow` 는 `DateTimeOffset.UtcNow` 다(`:93`).

### 2026-08-07 :: `_ResetStatics` 를 `#if !UNITY_EDITOR` 밖으로 이동

- 이전: `_ResetStatics` 가 `#if !UNITY_EDITOR` 블록 안에 있었다. Domain Reload 비활성은 에디터 전용 기능이라, 훅이 필요한 유일한 환경에서 정확히 존재하지 않았고 이전 플레이 세션의 `OnLogPublished` 구독자가 잔존했다.
- 현재: 훅 본체는 항상 컴파일되고(`:83-89`) `logQue.Clear()` 만 `#if !UNITY_EDITOR` 안에 있다.
