...
namespace HUtil.Editor.Subscription {
    public sealed class AssetOwnerIdWatcherWindow : EditorWindow {
        ...
        void _DrawBody() {
            ...
            IEnumerable<AssetOwnerIdWatchRegistry.Entry> entries = table.Values
            ...
            using (EditorGUILayout.ScrollViewScope scope = new(scrollPosition)) {
                ...
                foreach (AssetOwnerIdWatchRegistry.Entry entry in entries)
                    _DrawEntry(entry);
            }
        }

        void _DrawEntry(AssetOwnerIdWatchRegistry.Entry entry) {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.helpBox)) {
