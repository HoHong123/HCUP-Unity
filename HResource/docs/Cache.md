...
2. **스레드 안전하지 않다.** `Dictionary` 를 잠금 없이 쓴다. `AssetOwnerIdGenerator` 만 `Interlocked` 로 보호되어 있어 (`Subscription/AssetOwnerIdGenerator.cs:56`) id 발급은 스레드 안전하지만 캐시 조작은 메인 스레드 전제다.
