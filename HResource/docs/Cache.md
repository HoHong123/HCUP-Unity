...
> 대상: `Runtime/Cache/*.cs` (`IAssetReader` / `IAssetWriter` / `IAssetReleaser` / `IAssetCache` / `MemoryAssetCache` + 에디터 진단 6파일)
...
## 데이터 모델 - key 별 소유자 수 + owner 별 점유 집합

```csharp
// Cache/MemoryAssetCache.cs:41-51
sealed class Item {
    public TAsset Asset;
    public int OwnerCount;   // 이 key 를 잡고 있는 서로 다른 소유자 수. 획득 횟수가 아니다
}

readonly Dictionary<TKey, Item> assetTable = new();                   // key  → 항목
readonly Dictionary<AssetOwnerId, HashSet<TKey>> ownerTable = new();  // owner → 잡은 key 집합 (점유의 정본)
```

소유는 불린 관계다. "이 소유자가 이 key 를 살려둬야 하나" 는 예/아니오이고 "두 번 살려둬야 한다" 는 뜻이 없다. 횟수를 세면 소유자가 자기 획득 횟수를 기억해야 하는데, 그 기록을 들 자리가 없는 소유자가 있다 (`HDialogue` 의 `CharacterPortraitController` 는 `loadedSpriteKey` 필드 하나뿐이다). 그래서 중복 판정은 `ownerTable` 의 `HashSet<TKey>` 가 맡고, `Item` 은 제거 판정에 필요한 수만 든다. 두 값의 불변식은 "`OwnerCount` == 그 key 를 담은 `ownerTable` 집합 수" 다.

`ownerTable` 은 누가 무엇을 잡았는지의 유일한 기록이다. `ReleaseOwner` 는 전체 `assetTable` 을 순회하지 않고 **그 owner 가 잡은 key 만** 돈다 (`Cache/MemoryAssetCache.cs:134-158`). 루프 전에 key 마다 그때의 `Item` 을 기억해 두고, key 가 여전히 같은 `Item` 을 가리킬 때만 수를 내린다. 알림 도중 구독자가 `Clear` 후 같은 key 를 다시 `Save` 하면 새 `Item` 에는 이 owner 의 몫이 없기 때문이다.

### 제거 조건

```mermaid
flowchart TD
    A["Release 계열 호출"] --> B["ownerTable 에서 그 key 를 떼고 OwnerCount 감소"]
    B --> C["_TryRemoveItem"]
    C --> F{"OwnerCount &gt; 0"}
    F -->|예| E["유지 - false 반환"]
    F -->|아니오| G["assetTable.Remove"]
    G --> H["OnAssetRemoved 발화"]
    H --> I["AssetProvider._ReleaseTrackedLoader - 그 key 를 로드한 로더 하나만 해제"]
```

**`OwnerCount` 가 0 이 되면 제거된다.** 같은 소유자가 같은 key 를 여러 번 요청해도 상태는 바뀌지 않고, 한 번의 반납으로 끝난다.
...
| 메서드 | 대상 | 감소 방식 | 반환 |
|---|---|---|---|
| `Release(key, ownerId)` | 그 owner 의 점유 | **제거**. 유무라 한 번이면 끝난다 | 항목까지 제거됐으면 `true` |
| `ReleaseOwner(ownerId)` | 그 owner 의 전 key | **제거**. 단건 Release 와 같은 의미 | 내려놓은 key 수 |
| `ReleaseAll()` | 전체 | 소유자를 가리지 않고 전량 제거 | - |
| `Clear()` (`:164-166`) | 전체 | `ReleaseAll` 과 동일 구현 | - |
...
sequenceDiagram
    ...
    Note over M: OwnerCount = 1, ownerTable[idA] = {key}

    B->>P: GetAsync(B, key, loadMode)
    Note over P,M: 캐시 히트 - 로더 호출 없음
    P->>M: Save(key, asset, idB)
    Note over M: OwnerCount = 2, ownerTable[idB] = {key}

    A->>P: ReleaseOwner(A)
    P->>M: ReleaseOwner(idA)
    Note over M: OwnerCount = 1 - 아직 제거 안 됨
    M-->>P: 1

    B->>P: Release(B, key)
    P->>M: Release(key, idB)
    Note over M: OwnerCount = 0 → 제거
...
```csharp
// Cache/MemoryAssetCache.cs:82-109 - 요약
...
③ 이 provider 의 롤백 경로와 짝을 이룬다 - 거부되면 provider 가 방금 로드한 핸들을 그 요청의 로더에 직접 돌려준다 (`Provider/AssetProvider.cs:278-283`). silent overwrite 를 하지 않는 대신 호출자에게 `default` 를 돌려준다.
...
```csharp
// Cache/MemoryAssetCache.cs:223-247
...
if (assetTable.Count > 0) HLogger.Error("[AssetCache] Clear did not converge: ...");
```
...
`_NotifyRemoved` 는 구독자별로 `try/catch` 를 둔다 (`:331-345`). 구독자 하나가 던져도 나머지 구독자와 남은 항목의 알림은 계속된다. 끊기면 이미 `assetTable` 에서 빠진 항목의 로더 핸들이 아무도 회수하지 않는 상태로 남기 때문이다.
...
| `AssetOccupancySnapshot.cs` | key 하나의 총 점유와 소유자 목록 (Resource Ownership 탭) |
| `AssetHoldingSnapshot.cs` | 소유자 하나가 잡고 있는 key 목록 (Owner Tracker 탭) |
| `AssetOwnerOccupancy.cs` | key 를 잡고 있는 소유자 id |

`MemoryAssetCache` 가 `IAssetCacheDiagnostics` 를 구현하고, 생성자에서 `AssetCacheDiagnosticsRegistry` 에 자신을 **약한 참조**로 등록한다 (`Cache/MemoryAssetCache.cs:64-66`). 캐시는 provider 마다 `new` 로 만들어져 어디에도 등록되지 않으므로, 에디터 창이 살아 있는 캐시에 닿으려면 캐시가 스스로 손을 드는 수밖에 없다.

| 멤버 | 내용 |
| --- | --- |
| `CacheLabel` | 등록 시 발급된 표시용 문자열. 같은 제네릭 조합이 여러 개여도 순번으로 구분된다 |
| `EntryCount` | `assetTable.Count` |
| `CaptureOccupancy(buffer)` | 버퍼를 비우고 key 마다 `AssetOccupancySnapshot` 을 채운다. `ownerTable` 을 뒤집어 만든다 |
| `CaptureHoldings(buffer)` | 버퍼를 비우고 소유자마다 `AssetHoldingSnapshot` 을 채운다. `ownerTable` 을 그대로 옮긴다 |
| `ForceReleaseOwner(int)` | 그 소유자의 점유를 `ReleaseOwner` 로 강제 해제. 워처의 `Orphan Clean` 이 쓴다 |

스냅샷은 **방향마다 한 가지씩 두 모양**이다. 소유자 목록의 원본이 `ownerTable` 하나이므로 owner 기준은 그대로 옮기고, key 기준은 뒤집는다. key 기준 스냅샷의 `TotalCount` 는 캐시의 `OwnerCount` 이고 `Owners` 는 뒤집은 목록이라, 둘의 수가 다르면 두 인덱스가 어긋난 것이다. 워처는 탭마다 따로 `Scan` 하므로 두 탭이 서로 다른 순간을 보여줄 수 있다.

반환값이 아니라 호출자의 버퍼를 채운다. 캡처는 워처의 `Scan` 을 누를 때만 돈다. 그래도 한 번의 캡처 안에서는 뒤집기용 사전과 key 또는 소유자마다 리스트를 새로 만든다.
...
3. **`OnAssetRemoved` 의 구독자는 `AssetProvider` 하나다.** 구독 해제는 `AssetProvider.Dispose()` 에서 `ReleaseAll` 로 연쇄를 태운 뒤에 한다 (`Provider/AssetProvider.cs:217-230`). 순서를 뒤집으면 남은 점유의 로더 핸들이 반납되지 않는다.
...
### 2026-09-23 :: 소유자 집합을 수로 줄이고 진단 캡처를 두 방향으로

- 이전: `Item.Owners` 가 `HashSet<AssetOwnerId>` 였고 `ownerTable` 과 같은 관계를 반대 방향으로 들었다. 빌드에서 key 쪽이 쓰는 것은 `Owners.Count` 뿐이었다. 진단 스냅샷은 key 기준 한 가지였고 워처는 `OnGUI` 마다 캡처했다.
- 현재: `Item.OwnerCount` 다. 중복 판정과 소유자 목록은 `ownerTable` 이 맡는다. `ReleaseOwner` 는 key 가 여전히 같은 `Item` 을 가리킬 때만 수를 내린다. 알림 도중 `Clear` 후 재저장이 있어도 살아있는 소유자의 자산에 가짜 `OnAssetRemoved` 를 쏘지 않는다. 진단은 `CaptureHoldings` 가 더해져 두 방향이고, 워처는 `Scan` 을 누를 때만 캡처한다. 메인 테이블 이름은 `assetTable` 이고 `TAsset` 에 `class` 제약이 붙었다.
