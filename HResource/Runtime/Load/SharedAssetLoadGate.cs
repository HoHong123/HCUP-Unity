 ...
 *
 * 사용법 ::
 * AssetProvider 가 _GetAsync 에서 fetch mode 전 구간을 본 게이트로 감쌈. 로더 호출은 반드시 게이트를 지난다. CacheFirst 캐시 히트만 게이트 밖(provider 입구)에서 끝난다.
 *
 ...
 *
 * 이유 ::
 * AssetProvider 에 CacheFirst 캐시 히트 빠른 경로가 생겨 히트는 이 게이트를 지나지 않는다.
 * 이 게이트가 지키는 것은 "같은 key 로더 호출을 한 번으로 합쳐 참조 수를 1 로 고정" 이고, 히트는 로더를 부르지 않는다.
 *
 * 주의 ::
 * 로더를 부르는 경로가 새로 생기면 반드시 이 게이트를 지나게 한다. 그것이 여기서 지킬 유일한 불변식이다.
 *
 * =========================================================
 * 2026-09-22 (수정) :: HResource 테스트 어셈블리 제거
 *
 * 변경 ::
 * 코드 변경 없음. 사용자 지시로 HResource/Tests 를 통째로 지웠다.
 *
 * 주의 ::
 * 아래 (검증) 항목과 그 아래 항목들이 가리키는 HCUP.HResource.Tests 는 더 이상 저장소에 없다.
 * 실행 기록은 그 시점의 사실로 남긴다. 테스트 원문은 제거 직전 커밋 3472218 에 있다.
 *
 * =========================================================
