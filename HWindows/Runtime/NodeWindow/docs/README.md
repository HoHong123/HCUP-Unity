...
Editor/NodeWindow/
  Core/
    ...
    HGraphNodeStyles.cs      - 헤더 색 상수 + 도메인 노드 타입별 헤더 색 레지스트리
  NodeCatalog/
    Authoring/
...
Runtime/NodeWindow/
  AssemblyInfo.cs            - InternalsVisibleTo("HCUP.HWindows.NodeWindow.Editor")
  NodeCatalog/
...
| `BaseNode` | Runtime | 노드 데이터 ScriptableObject. `editorPosition`, `editorFoldoutOpen` 에디터 상태 포함 (`#if UNITY_EDITOR`) |
...
| Active 하이라이트 | `HGraphCanvas.HighlightActiveNode(uid)` | Play 모드에서 파생 창이 런타임 이벤트를 구독해 현재 노드를 강조합니다 (`HGraphCanvas.cs:812-828`) |
| Trace 모드 | `HGraphCanvas.SetTraceMode(bool)` / `TracePathFrom(rootUid)` | 지정 노드에서 이어지는 경로를 표시합니다 (`HGraphCanvas.cs:850-898`) |
...
1. **Experimental API 격리** - `UnityEditor.Experimental.GraphView`를 직접 `using` 하는 파일은 `Core/HGraphCanvas.cs`, `Core/HGraphNode.cs`, `Core/HGraphCatalogNode.cs`, `Core/HGraphHubNode.cs`, `Core/HGraphEdge.cs` 다섯 파일로 제한합니다. 외부 코드는 `HWindows.Editor.NodeWindow` 네임스페이스만 소비합니다.
...
3. **HCUP 계층 경계** - Runtime 어셈블리는 `HCUP.HCollection`, `HCUP.HInspector` 를, Editor 어셈블리는 여기에 더해 `HCUP.HInspector.Editor`, `HCUP.HUtil`, `HCUP.HUtil.Editor`, `HCUP.HDiagnosis` 를 참조합니다. `HGame`, `HUI`를 역방향 참조하지 않습니다.
...
static void _Register() {
    ...
    HGraphCanvas.RegisterNodeViewFactory(
        typeof(MyLineNode),
        (node, isRoot) => new HGraphNode(node, isRoot));  // 둘째 인자는 루트 여부. 커스텀 뷰로 교체 가능
...
- 이 패키지는 `TextField.SetPlaceholderText` 를 쓰지 않습니다. Unity 2022.3 에서 CS1061 컴파일 오류가 나는 API 라 새로 쓸 때는 `#if UNITY_6000_0_OR_NEWER` 로 분기합니다.
- `package.json`의 `"unity": "2021.3"` 항목은 패키지 배포 호환 선언입니다. 6000.3 에서 개발·검증합니다. `NodeCatalogObjectChangeWatcher` 는 `#if UNITY_6000_3_OR_NEWER` 로 `EntityIdToObject` 와 `InstanceIDToObject` 를 나눠 씁니다.
...
### 2026-09-21 :: 코드와 어긋난 서술 정정

- 이전: 노드 뷰 팩토리 예시가 `(node, catalog) => new HGraphNode(node, catalog)` 였습니다. 디렉토리 구성이 `NodeUIDDrawer.cs` 를 `NodeCatalog/Identity/` 아래로 적었습니다. Active 하이라이트·Trace 모드가 기능 표에 없었습니다. Experimental API 를 쓰는 파일을 2개로, `HWindows` 의 의존을 `HUtil` 하나로 적었습니다. `BaseNode` 의 에디터 상태 필드를 `isFoldoutOpen` 으로, `HGraphNodeStyles` 를 USS 클래스 상수로 적었습니다.
- 현재: 팩토리는 `Func<BaseNode, bool, HGraphNode>` 이고 둘째 인자는 `isRoot` 입니다 (`HGraphCanvas.cs:63`, `:598`, `HGraphNode.cs:77`). `NodeUIDDrawer.cs` 는 `Editor/NodeWindow/Identity/` 에 있습니다. 나머지 항목도 현재 코드 기준으로 고쳤습니다.

### 2026-05-11 :: `TextField.SetPlaceholderText` 사용 제거

- 이전: 주의사항이 `SetPlaceholderText` 를 `#if UNITY_6000_0_OR_NEWER` 분기 또는 사용 제거로 처리한다고만 적어, 현재 사용 여부가 드러나지 않았습니다.
- 현재: `HGraphWindow` 가 CS1061 을 피하려고 이 API 호출을 제거했습니다 (`HGraphWindow.cs` Dev Log `LOG-20260511-1`).
