 ...
 * Dev Log
 * =========================================================
 * 2026-08-07 (수정) :: NumToAlpha K 단위 경계 정정
 *
 * 변경 ::
 * - K(천) 단위 분기 조건 `num >= 10_000` → `num >= 1_000` 로 수정.
 *
 * 이유 ::
 * 나머지 단위(K/M/B/T)는 모두 "그 단위의 기준값 이상이면 그 단위로 나눈다" 는 규칙인데
 * (M: >= 1_000_000, B: >= 1_000_000_000, T: >= 1_000_000_000_000), K 만 나누는 값은
 * 1_000 이면서 분기 조건은 10_000 이었다. 그 결과 1,000~9,999 구간이 축약되지 않고
 * 그대로 표시되며, 10,000 은 "10.0K/10.0천" 으로 표시되는 등 단위 경계가 어긋났다.
 * =========================================================
 */
#endif
