...
HData 는 **의존이 하나도 없는 최하위 유틸 어셈블리**다. 이 저장소 안에서는 `HCUP.HCore` 와 `HCUP.HAudio.Editor` 가 이 어셈블리를 직접 참조하고, `HCUP.HCore` 를 거쳐 그 위 패키지들이 간접 의존한다.
...
| `Mathx/VectorUtil.cs` | 77 | `HData.Mathx` | RectTransform 랜덤 위치 / 스크린 좌표 / 각도→방향 |
| `Primitives/EnumUtil.cs` | 17 | `HData.Primitives` | `GetValues<T>()` 한 개 |
| `Primitives/FloatUtil.cs` | 17 | `HData.Primitives` | `MidAngleDegree` 한 개 |
| `Primitives/JTokenUtil.cs` | 39 | `HData.Primitives` | Newtonsoft `JToken` 확장 - **컴파일되지 않는다** (아래 참조) |
| `Primitives/StringUtil.cs` | 178 | `HData.Formattable` | 쿼리 파싱 / Base64 / 숫자·시간 포맷 / 필터 |
...
flowchart TD
    ...
    subgraph Up["상위"]
    HC["HCUP.HCore"]
    HAE["HCUP.HAudio.Editor"]
    end
    HC --> HData
    HAE --> HData
```

`Encode` 와 `Encrypt` 는 **서로를 모른다.** "암호화 후 Base64 로 저장" 파이프라인은 상위 계층에서 두 인터페이스를 조합해 만들어야 한다.
...
**`NumToAlpha` 는 각 단위의 기준값 이상이면 축약한다.**

```csharp
// Primitives/StringUtil.cs:85-88 - 마지막 분기가 1_000 이다
else if (num >= 1_000)
...
`1,000` 은 `"1.0K"` / `"1.0천"` 이 된다. 한국어 단위는 천·백만·십억·조로, 영문 K/M/B/T 경계를 그대로 번역한 것이라 "만" 단위가 없다. `10,000` 은 `"10.0천"` 이다.
...
8. **`EnumUtil` / `FloatUtil` 은 메서드 1개짜리 파일이다** (17행씩). 한 파일로 합칠 여지가 있다.
9. **`StringUtil` 만 네임스페이스가 `HData.Formattable` 이다** (`:21`). 폴더는
...
- 이전: `GetCanvasPosition` 은 이름과 달리 캔버스 좌표가 아니라 `Camera.WorldToScreenPoint` 결과를 반환했고, 파라미터명이 `_target` / `_camera` 로 컨벤션을 벗어났다.
- 현재: `GetScreenPosition(this Transform target, Camera camera)` 로 개명해 이름과 동작(World→Screen 변환)을 맞췄다 (`VectorUtil.cs:33-35`).

### 2026-08-07 :: `StringUtil.NumToAlpha` 의 K 단위 경계 정정

- 이전: 마지막 분기가 `num >= 10_000` 이라 `1,000` ~ `9,999` 는 `"9999"` 처럼 축약되지 않았고 `10,000` 부터 `"10.0K"` 였다.
- 현재: `num >= 1_000` 으로 정정해 다른 단위와 같은 "기준값 이상" 규칙을 따른다.
