...
5개 모두 같은 문제를 푼다. **런타임 컴포넌트가 `[HideInInspector]` 로 감춰 둔 직렬화 필드를, 기본 인스펙터를 유지한 채 노출한다.**
...
| 경로 | 대상 | 상속한 에디터 | 그리는 것 |
|---|---|---|---|
| `HUI/Image/HImageEditor.cs` | `HUI.ImageUI.HImage` | `UnityEditor.UI.ImageEditor` | Alpha Hit Test 2필드 + 정렬 옵션 2필드 + 액션 버튼 3개 |
...
로컬라이제이션 에디터 2종(`HTextEditor` / `HTmpTextEditor`)과 `HImageEditor` 의 `base.OnInspectorGUI()` 호출 위치가 반대다. 애드온 에디터 2종은 `base.OnInspectorGUI()` 를 부르지 않는다.
...
3. **애드온 에디터 2종은 `base.OnInspectorGUI()` 를 부르지 않는다.** 따라서 스크립트 필드 슬롯 없이 4개 프로퍼티만 보인다. 의도된 축약이다 (`HTextLocalizerAddonEditor.cs:31-47`).
...
6. **`ScrollView` / `Popup` / `DropDown` / `DebugConsole` 에는 에디터 지원이 전혀 없다.**
   `BaseRecycleView` 는 `viewport`·`content`·`itemPrefab` 배선이 전부 필수인데
   (`BaseRecycleView.cs:61-70`), 누락을 잡아 주는 인스펙터 검증이 없다. 실패는 런타임
