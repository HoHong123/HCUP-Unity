#if UNITY_EDITOR
/* =========================================================
 * 이 스크립트는 가로 스크롤 전용 RecycleView를 담당하는 스크립트입니다.
 * 수평 방향의 ScrollRect 환경에서 셀을 재사용하며, 현재 보이는 범위의 셀만 생성 및 유지합니다.
 *
 * 주의사항 ::
 * 1. 셀의 가로 크기(itemWidth), 간격(spacing), 좌우 패딩 값은 실제 UI 레이아웃과 일치해야 합니다.
 * 2. virtualization 동작은 content 크기와 viewport 크기 비교를 기준으로 판단됩니다.
 * 3. ScrollToIndex 호출 시 index 범위가 유효하지 않으면 동작하지 않습니다.
 * 4. 셀 위치 계산은 좌측부터 순차 배치되는 구조를 전제로 합니다.
 * 5. BaseRecycleView 내부 필드(scrollRect, content, viewport, itemPool, activeItems, dataList)에 정상 접근 가능해야 합니다.
 * 6. Context의 가로 길이는 가변적입니다. 세로 길이만 의도에 맞게 수정하세요.
 * ********** 중요 **********
 *  7. 사용되는 Item UI와 Context 모두 좌상 Anchore와 Pivot (0, 1)로 사용하셔야 합니다. 
 * =========================================================
 */
#endif
...
namespace HUI.ScrollView {
    ...
    public class HorizontalRecycleView<TCellView, TCellData> : BaseRecycleView<TCellView, TCellData>, IRecycleView
        ...
        public float TotalContentSize {
            get {
                float itemsWidth = Mathf.Max(0f, (itemWidth + spacing) * Count - spacing);
                return leftPadding + itemsWidth + rightPadding;
            }
        }
        ...
        public override void ScrollToIndex(int index, bool center = true) {
            ...
            float targetX = leftPadding + index * itemSpace - centerOffset;
        ...
        protected override void UpdateContentSize() {
            float itemsWidth = Mathf.Max(0f, (itemWidth + spacing) * Count - spacing);
            float totalWidth = leftPadding + itemsWidth + rightPadding;
        ...
        protected override void UpdateVisibleItems() {
            ...
            float offset = scrollX - leftPadding;
            if (offset < 0f) offset = 0f;

            int start = Mathf.Max(0, Mathf.FloorToInt(offset / (itemWidth + spacing)));
        ...
        protected override void CreateCell(int index) {
            ...
            rect.anchoredPosition = new Vector2(leftPadding + index * (itemWidth + spacing), 0);
 ...
 *
 * 주요 기능 ::
 * 1. Horizontal ScrollRect 전용 RecycleView 동작을 제공합니다.
 * 2. TotalContentSize를 통해 전체 컨텐츠 가로 길이를 계산합니다.
 ...
 *
 * 사용법 ::
 * 1. itemWidth, spacing, leftPadding, rightPadding을 Inspector에서 설정합니다.
