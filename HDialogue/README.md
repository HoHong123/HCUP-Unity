...
> 모듈: `HDialogue/` · 소스 75파일 · `package.json` v1.0.0 (`com.hohong123.hdialogue`)
> 구성 어셈블리 2개
...
**이 저장소에서 asmdef 참조가 가장 많은 모듈이다.** 런타임 참조 14개 중 10개가 HCUP 내부 모듈이라, HDialogue 를 쓰려면 사실상 저장소 전체가 따라온다.

---

## 설치 · 요구 사항

저장소를 통째로 가져다 쓴다. 개별 UPM 설치는 현재 동작하지 않는다 ([루트 README 의 설치 절](../README.md#설치) 참조).
...
1. **`docs/TagUsage.md` 는 인라인 이벤트 라우팅을 사실과 다르게 서술한다.** 문서는 `<event=sfx.coin>` 이 SFX 를 울린다고 적었지만, 패키지 안에서 인라인 태그(`DialogueTextController.OnEventTagFired`)를 구독하는 것은 `CharacterStageDirector` 하나뿐이고 `portrait.` 접두가 아니면 폐기된다. `DialogueAudioController` 는 그래프의 `DialogueEventNode` 가 발행하는 `DialogueDirector.OnEventFired` 만 구독한다. **패키지 기본 구성으로는 인라인 태그로 SFX 를 울릴 방법이 없다.** `Face`/`Slot` 인자값, `Show` 인자 유무, W003 정의 등도 코드와 어긋나 있다.
...
4. **설계만 남고 런타임이 비어 있는 값들이 있다.** `WaitNode.conditionKey`, `PortraitTransitionType.SlideIn`/`Scale`, `PortraitPoseType.Sequence`, `DialogueTokenType.Sfx`. 인스펙터에서 고를 수 있지만 동작하지 않는다.
